...
export default function AdminPage() {
  ...
  const modeLabel = mode === "post" ? "Blog" : "Notes";
      ...
      <div className="mx-auto grid w-full max-w-7xl gap-5 lg:grid-cols-[280px_minmax(0,1fr)]">
        ...
        >
          <div className="flex rounded-full border border-white/10 bg-white/5 p-1">
            {(["post", "note"] as ContentMode[]).map((item) => (
              <button
                key={item}
                type="button"
          ...
          <div className="mt-5 space-y-2">
            ...
            {items.length === 0 ? <p className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm text-starlight/45">暂无内容或数据库未连接。</p> : null}
          </div>
        </motion.aside>
        ...
        >
          <header className="flex flex-col gap-4 border-b border-white/10 pb-5 md:flex-row md:items-end md:justify-between">
            <div>
              ...
              <h1 className="mt-3 font-display text-4xl text-starlight sm:text-5xl">星际内容管理舱</h1>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              >
                {isSaving ? "保存中" : form.published ? "发布" : "保存草稿"}
              </button>
            </div>
          </header>
